...
description: "ExceptionsUtils provides lightweight guard helpers that raise descriptive exceptions when required keys are missing from dictionaries or kwargs."
---

`ExceptionsUtils` is a static-method utility class that centralises the repetitive pattern of validating required keys before a function proceeds. Rather than scattering `if key not in options: raise Exception(...)` blocks across your codebase, you delegate that check to a single, consistently worded guard. Kimera Core uses this internally — for example, `RawZipExtractor` calls `raise_exception_if_key_not_in_dict('filename', options)` to assert that callers always supply the required `filename` key before any file I/O begins. The resulting error messages are human-readable and actionable, making misconfiguration obvious at development time.

## `raise_exception_if_key_not_in_dict`

Checks whether `key` is present in `_dict` and raises an `Exception` with a descriptive message if it is not. The check uses Python's `in` operator, so it works with any mapping type including plain `dict` and `**kwargs`.

<ParamField path="key" type="str" required>
  The key that must be present in the dictionary. Used verbatim in the error message.
</ParamField>

<ParamField path="_dict" type="dict" required>
  The dictionary (or unpacked `**kwargs`) to inspect. No exception is raised when `key` is found.
</ParamField>
