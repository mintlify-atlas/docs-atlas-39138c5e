---
title: "Logger: Structured Logging with Logstash Support"
sidebarTitle: "Logger"
...
Serialises the log record by calling `self.format(record)` (which applies the configured formatter) and encoding the resulting string as UTF-8 bytes. The return value is transmitted directly as the UDP payload.

## `DICT_CONFIG`

`DICT_CONFIG` is a fully populated `logging.config.dictConfig`-compatible dictionary that can be used as-is or extended. Its structure follows the standard Python logging schema:
...
<Accordion title="Formatters (3 defined)">
  ...
  | `file` | Applied to the (commented-out) file handler |
...
<Accordion title="Loggers (2 defined)">
  Both `logger1` and `logger2` are configured identically: level `DEBUG`, `propagate: True`, and both `stream_handler` and `logstash_handler` attached.
</Accordion>
