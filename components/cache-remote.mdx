...
<AccordionGroup>
  ...
  <Accordion title="set(key: str, value: object, timeout: float = None, size: int = None) -> None">
    Calls `redis.set(key, value, timeout)`. The `timeout` value is passed directly to the Redis driver as the `ex` (expiry in seconds) argument. The `size` parameter is accepted for interface compatibility but is not used.
  </Accordion>
...
<Warning>
  The remote engines do **not** yet support `size` regulation or normalized TTL conversion like the local engines. The `timeout` value is passed directly to the backend driver (Redis `set(..., ex=timeout)` or Memcached, which currently ignores it). Do not rely on size-based eviction or TTL when using remote adapters until these features are implemented.
</Warning>
