...
<AccordionGroup>
  <Accordion title="serialize(object) -> str">
    Selects the appropriate handler for the object and returns a JSON string envelope containing `{'data': ..., 'serializer': '<HandlerClassName>'}`.
  </Accordion>
  <Accordion title="deserialize(str) -> object">
    Reads the `serializer` field from the JSON envelope to identify the correct handler and reconstructs the original Python object.
  </Accordion>
</AccordionGroup>
...
<Tip>
  `DateSerializerHandler` must appear **before** `DateTimeSerializerHandler` in the chain because `datetime` is a subclass of `date`. Reversing the order would cause all `date` objects to be incorrectly matched by the `datetime` handler.
</Tip>
